//! Operator commands for a DiscordFS deployment.
//!
//! ```text
//! discordfs config-check          # read the environment and say what would happen
//! discordfs status [--server URL] # ask a running server how it is
//! discordfs put FILE [--resume]   # upload a local file, carrying on if it broke
//! ```

use std::process::ExitCode;

const HELP: &str = "\
Usage: discordfs <command>
...
Commands:
  ...
  status         Ask a running server for its health and part size.
                 --server URL   default $DISCORDFS_SERVER or http://127.0.0.1:8080
                 The bearer token comes from $DISCORDFS_TOKEN or $API_TOKEN.
...
async fn main() -> ExitCode {
    ...
    match args.next().as_deref() {
        ...
        Some(other) => {
            eprintln!("discordfs: unknown command {other:?}\n\n{HELP}");
            ExitCode::from(2)
        }
    }
}
...
fn config_check() -> ExitCode {
    let config = match discordfs_server::config::Config::from_env() {
        Ok(config) => config,
        Err(e) => {
...
async fn status(mut args: impl Iterator<Item = String>) -> ExitCode {
    let mut server =
        std::env::var("DISCORDFS_SERVER").unwrap_or_else(|_| "http://127.0.0.1:8080".to_string());
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--server" => match args.next() {
                Some(url) => server = url,
                None => {
                    eprintln!("discordfs: --server needs a URL");
                    return ExitCode::from(2);
                }
            },
            other => {
                eprintln!("discordfs: unknown option {other:?}");
                return ExitCode::from(2);
            }
        }
    }
    // Only from the environment: a token on the command line shows up in `ps`.
    let token = std::env::var("DISCORDFS_TOKEN")
    ...
    let Some(token) = token else {
        println!("part size: needs a token; set DISCORDFS_TOKEN");
...
async fn put(mut args: impl Iterator<Item = String>) -> ExitCode {
        ...
        match arg.as_str() {
            "--server" | "--name" => {
                let Some(value) = args.next() else {
                    eprintln!("discordfs: {arg} needs a value");
                    return ExitCode::from(2);
                };
                if arg == "--server" {
                    server = value;
    ...
    let Some(path) = path else {
        eprintln!("discordfs: put needs a file\n\n{HELP}");
        return ExitCode::from(2);
    };
    // Only from the environment: a token on the command line shows up in `ps`.
    let Some(token) = std::env::var("DISCORDFS_TOKEN")
        .or_else(|_| std::env::var("API_TOKEN"))
        .ok()
    else {
        eprintln!("discordfs: set $DISCORDFS_TOKEN or $API_TOKEN");
        return ExitCode::FAILURE;
    };
    ...
    let total = match std::fs::metadata(local) {
        Ok(meta) => meta.len(),
        Err(e) => {
            eprintln!("discordfs: cannot read {path}: {e}");
            return ExitCode::FAILURE;
        }
    };
    ...
    let part_size = match auth(http.get(format!("{server}/api/v1/fs"))).send().await {
        Ok(resp) => match resp.json::<serde_json::Value>().await {
            ...
            Err(e) => {
                eprintln!("discordfs: cannot read part size: {e}");
                return ExitCode::FAILURE;
            }
        },
        Err(e) => {
            eprintln!("discordfs: cannot reach {server}: {e}");
            return ExitCode::FAILURE;
        }
    };
    ...
    let (node_id, mut offset) = match existing {
        Ok(resp) if resp.status().is_success() => {
            let node: serde_json::Value = match resp.json().await {
                Ok(node) => node,
                Err(e) => {
                    eprintln!("discordfs: {name} is there but unreadable: {e}");
                    return ExitCode::FAILURE;
                }
            };
            if !resume {
                eprintln!(
                    "discordfs: {name} already exists ({} bytes); pass --resume to carry on",
        ...
        _ => {
                ...
                .await
            {
                Ok(resp) => match resp.json::<serde_json::Value>().await {
                    ...
                    Err(e) => {
                        eprintln!("discordfs: cannot read the root: {e}");
                        return ExitCode::FAILURE;
                    }
                },
                Err(e) => {
                ...
                .await
            {
                Ok(resp) if resp.status().is_success() => {
                    match resp.json::<serde_json::Value>().await {
                        ...
                        Err(e) => {
                            eprintln!("discordfs: cannot read the new file: {e}");
                            return ExitCode::FAILURE;
                        }
                    }
                }
                Ok(resp) => {
                    eprintln!("discordfs: cannot create {name}: HTTP {}", resp.status());
                    return ExitCode::FAILURE;
                }
                Err(e) => {
                    eprintln!("discordfs: cannot create {name}: {e}");
                    return ExitCode::FAILURE;
                }
            }
        }
    ...
    let mut file = match std::fs::File::open(local) {
        Ok(file) => file,
        Err(e) => {
            eprintln!("discordfs: cannot open {path}: {e}");
            return ExitCode::FAILURE;
        }
    };
    if let Err(e) = file.seek(std::io::SeekFrom::Start(offset)) {
        eprintln!("discordfs: cannot seek {path}: {e}");
        return ExitCode::FAILURE;
    }
    ...
    while offset < total {
        ...
        while filled < buf.len() {
            match file.read(&mut buf[filled..]) {
                ...
                Ok(n) => filled += n,
                Err(e) => {
                    eprintln!("discordfs: reading {path} failed at {offset}: {e}");
                    return ExitCode::FAILURE;
                }
            }
        }
        ...
        if !sent {
            eprintln!(
                "discordfs: giving up at {offset}; rerun with --resume to carry on from here"
            );
            return ExitCode::FAILURE;
        }
        ...
        .await
    {
        ...
        Ok(resp) => {
            eprintln!(
                "discordfs: upload finished but sync returned HTTP {}",
                resp.status()
            );
            ExitCode::FAILURE
        }
        Err(e) => {
            eprintln!("discordfs: upload finished but sync failed: {e}");
            ExitCode::FAILURE
        }
    }
}
