...
use dcfs_objectstore::memory::MemoryObjectStore;
use dcfs_objectstore::{ObjectLocator, ObjectStore};
...
async fn put_and_get_roundtrip() {
    ...
    let _: dcfs_objectstore::StoredObject = store.put(id, data.clone()).await.unwrap();
...
async fn put_rejects_duplicate_id() {
    ...
    let _: dcfs_objectstore::StoredObject = store.put(id, data1).await.unwrap();
    let result: Result<dcfs_objectstore::StoredObject, _> = store.put(id, data2).await;
...
async fn delete_removes_object() {
    ...
    let _: dcfs_objectstore::StoredObject = store.put(id, data).await.unwrap();
    let locator = ObjectLocator::new(id);
...
async fn stat_returns_metadata() {
    ...
    let stored: dcfs_objectstore::StoredObject = store.put(id, data.clone()).await.unwrap();
    assert_eq!(stored.id, id);
    assert_eq!(stored.size, data.len() as u64);

    let locator = ObjectLocator::new(id);
    let stat: dcfs_objectstore::StoredObject = store.stat(&locator).await.unwrap();
...
async fn stat_returns_not_found_for_missing() {
    ...
    let result: Result<dcfs_objectstore::StoredObject, _> = store.stat(&locator).await;
    assert!(result.is_err());
}
...
async fn objects_are_immutable() {
    ...
    // Cannot overwrite
    let result: Result<dcfs_objectstore::StoredObject, _> =
