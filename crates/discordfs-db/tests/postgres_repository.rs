...
use discordfs_db::{CommitGuard, MetadataRepository, PgRepository, RepositoryError};
...
async fn installs_into_a_named_schema_without_touching_anything_else() {
    ...
    // DiscordFS installs itself into its own schema.
    ...
    // The other application's table is untouched, and DiscordFS did not land
    // in `public`.
    let note: String =
    ...
    let in_public: Option<String> = sqlx::query_scalar("SELECT to_regclass('public.nodes')::text")
        ...
        .await
        .unwrap();
    assert!(in_public.is_none(), "DiscordFS must stay out of public");
...
async fn object_locators_survive_a_restart() {
    ...
    repo.put_object_locator(&discordfs_db::ObjectLocatorRecord {
        object_id,
