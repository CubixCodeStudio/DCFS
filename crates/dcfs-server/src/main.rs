...
use dcfs_db::{MemoryMetadataRepository, MetadataRepository, PgRepository};
use dcfs_discord::{DiscordClient, DiscordClientConfig, DiscordObjectStore};
use dcfs_objectstore::{FsObjectStore, ObjectStore};
use std::sync::Arc;
use std::time::Duration;

use dcfs_server::{build_router, config::Config, locators::RepositoryLocatorStore, AppState};
...
async fn main() {
    ...
    dcfs_server::gc::spawn(repo, store, config.gc_retention, gc_interval);
    ...
    tracing::info!("DCFS server listening on {}", config.server_addr);

    axum::serve(listener, app).await.expect("server error");
}

/// Connect to PostgreSQL and make sure DCFS's tables are reachable.
///
