...
pub struct Config {
    ...
    /// PostgreSQL schema DCFS lives in. Lets it share a database with
    ...
    pub fn from_env() -> Result<Self, ConfigError> {
        ...
        let max_attachment_bytes = match optional_var("MAX_ATTACHMENT_BYTES")? {
            ...
            Some(raw) => {
                ...
                let sealed = chunk_size + dcfs_crypto::CIPHERTEXT_OVERHEAD;
                if sealed > max {
                    return Err(ConfigError::Invalid {
                        var: "CHUNK_SIZE",
                        reason: format!(
                            ...
                            max.saturating_sub(dcfs_crypto::CIPHERTEXT_OVERHEAD)
                        ),
                    });
                }
                Some(max)
