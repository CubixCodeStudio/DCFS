...
fn node_to_response(record: dcfs_db::NodeRecord) -> NodeResponse {
    ...
    NodeResponse {
        ...
        name: dcfs_protocol::NameBytes(record.name),
        kind,
        ...
        link_target: record.link_target.map(dcfs_protocol::NameBytes),
    }
}

/// Report how the server chunks files.
pub async fn get_fs_info(State(state): State<AppState>) -> Json<dcfs_protocol::FsInfoResponse> {
    Json(dcfs_protocol::FsInfoResponse {
...
pub async fn lookup_child(
    ...
    let name = dcfs_protocol::NameBytes::from_base64(&name)
