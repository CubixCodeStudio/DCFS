...
pub async fn upload_chunk(
    ...
    let response = UploadChunkResponse {
        object_id,
        ciphertext_size: body.len() as u64 + dcfs_crypto::ciphertext_overhead(body.len() as u64),
    };
...
pub async fn get_version(
    State(state): State<AppState>,
    Path(version_id): Path<Uuid>,
) -> Result<Json<dcfs_protocol::CommitVersionResponse>, AppError> {
    let version = state.repo.get_version(version_id).await?;
    let response = dcfs_protocol::CommitVersionResponse {
        version_id: version.id,
        node_id: version.node_id,
