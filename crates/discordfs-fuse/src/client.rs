//! Server client abstraction for DiscordFS API.
...
/// Abstract interface to the DiscordFS server.
///
...
fn default_retry() -> discordfs_core::RetryConfig {
    discordfs_core::RetryConfig::new()
        .with_max_attempts(6)
...
pub struct HttpClient {
    ...
    base_url: String,
    retry: discordfs_core::RetryConfig,
}
...
impl HttpClient {
    ...
    pub fn with_retry(mut self, retry: discordfs_core::RetryConfig) -> Self {
        self.retry = retry;
        self
    }
...
impl ServerClient for HttpClient {
    ...
    async fn find_child(&self, parent_id: Uuid, name: &[u8]) -> Result<NodeResponse, ClientError> {
        let encoded = discordfs_protocol::NameBytes(name.to_vec()).to_base64();
