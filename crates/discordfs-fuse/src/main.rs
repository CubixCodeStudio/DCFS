//! DiscordFS FUSE mount binary.
//!
//! ```text
//! discordfs-fuse <mountpoint> [--server URL] [--allow-other] [--read-only]
//! ```

fn main() {
    let mut mountpoint = None;
    let mut mode = std::env::var("DISCORDFS_MODE").unwrap_or_else(|_| "stream".to_string());
    let mut cache_dir = std::env::var("DISCORDFS_CACHE_DIR").ok();
    let mut cache_size = std::env::var("DISCORDFS_CACHE_SIZE").ok();
    let mut server =
    ...
    let token = std::env::var("DISCORDFS_TOKEN").ok();
    ...
    let mode = match mode.as_str() {
        "stream" => discordfs_fuse::Mode::Stream {
            budget_bytes: cache_size,
        },
        "mirror" => discordfs_fuse::Mode::Mirror,
        other => fail(&format!(
    ...
    let cache_dir = cache_dir.unwrap_or_else(|| {
        std::env::temp_dir()
            .join(format!("discordfs-cache-{}", std::process::id()))
            .to_string_lossy()
            .into_owned()
    });
...
const HELP: &str = "\
Usage: discordfs-fuse <mountpoint> [options]

Options:
  --server URL      Server to mount. Default $DISCORDFS_SERVER or http://127.0.0.1:8080
  --mode MODE       stream (default) or mirror, see below. $DISCORDFS_MODE
  --cache-dir PATH  Where cached file contents live. $DISCORDFS_CACHE_DIR
  --cache-size N    Stream-mode cache budget in bytes, default 1073741824 (1 GiB).
                    $DISCORDFS_CACHE_SIZE
...
The bearer token is read from $DISCORDFS_TOKEN, never from argv.
...
fn fail(message: &str) -> ! {
    eprintln!("discordfs-fuse: {message}");
    std::process::exit(2);
}

#[cfg(target_os = "linux")]
fn run(options: Options) {
    use discordfs_fuse::{BlockCache, DiscordFs, Fs, HttpClient, WriteLog};
    ...
    let mut mount_options = vec![
        ...
        MountOption::Subtype("discordfs".to_string()),
        MountOption::NoAtime,
    ];
