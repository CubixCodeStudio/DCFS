...
use discordfs_fuse::{ClientError, Fs, ROOT_INO};
use std::sync::Arc;
...
async fn small_sequential_writes_are_coalesced_until_flushed() {
    use discordfs_fuse::ServerClient;
...
use discordfs_fuse::{BlockCache, Mode};
...
async fn stream_mode_keeps_the_cache_inside_its_budget() {
    ...
    let (fs, cache) = mounted_with_cache(Mode::Stream {
        budget_bytes: discordfs_fuse::BLOCK_SIZE,
    })
    .await;
    ...
    assert!(
        cache.bytes() <= discordfs_fuse::BLOCK_SIZE,
